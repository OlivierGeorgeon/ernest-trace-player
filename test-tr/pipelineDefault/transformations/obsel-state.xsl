<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:math="http://exslt.org/math" extension-element-prefixes="math dyn exsl"
	version="1.1">

	<xsl:variable name="initial-state"
		select="exsl:node-set(document($state-filename))" />

	<!-- This stylesheet fragment is made to make easy transforming obsels, 
		and helps managing states. It sorts and groups incoming obsels by date and 
		call iteratively templates on the groups, while updating the state for each 
		group with the saves made by the transformations on the previous group. -->
	...
	<xsl:template match="/">
		<deltas>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<delta source="{$name}" date="{math:max(/deltas/delta/@date)}" />
				</xsl:otherwise>
			</xsl:choose>
		</deltas>
	</xsl:template>
	...
	<xsl:template name="fold-applying-templates">
		...
		<xsl:if test="$nodes[1]">
			...
			<xsl:variable name="next-nodes"
				select="$nodes[position() &gt; count($group-set/delta)]" />
			...
			<!-- Extract saves -->
			<xsl:variable name="save-names"
				select="exsl:node-set($outputs)/save[not(@name = preceding-sibling::save/@name)]" />
			<xsl:variable name="old-saves"
				select="$state/save[@name and not(@name = $save-names/@name)]" />
			...
			<xsl:choose>
				...
				<!-- If there are no more nodes to process, save the state to a file 
					and return. -->
				<xsl:otherwise>
	...
	<xsl:template name="exec-manage-saves">
		...
		<!-- Manage named saves -->
		<xsl:variable name="save-names"
			select="$node-output/save[@name and not(@name = preceding-sibling::save/@name)]" />
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="new-save-names"
					select="$node-output/save[not(@name = preceding-sibling::save/@name)]" />
				<!-- Excluding unnamed saves -->
				<xsl:copy-of
					select="$state/save[@name and not(@name = $new-save-names/@name)]" />
