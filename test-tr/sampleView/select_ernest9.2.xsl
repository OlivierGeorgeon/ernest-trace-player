...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="sequence">
		<xsl:element name="sequence">
			...
			-->
			<xsl:variable name="group-obs-color">
				<xsl:call-template name="group-adjacent">
					<xsl:with-param name="nodeset" select="event[@source='Ernest']" />
					<xsl:with-param name="equal-path" select="string('current_observation/color')" />
					<xsl:with-param name="sort-path" select="string('@date')" />
					<xsl:with-param name="date-path" select="string('@id')" />
				</xsl:call-template>
			</xsl:variable>
			...
			<xsl:for-each select="exsl:node-set($group-obs-color)/group">
				<xsl:variable name="beginid" select="@begin" />
				<xsl:variable name="endid" select="@end" />
				<obsel id="{$beginid}2" date='{$ernests[@id = $beginid]/@date}'
					begin='{$ernests[@id = $beginid]/@date}' end='{$ernests[@id = $endid]/@date}'>
					<type>color_percent_to_target</type>
					...
					<xsl:variable name="dist">
						<xsl:choose>
							<xsl:when test="$ernests[@id = $crave-group/@begin]/is_hungry = 'true'">
								<xsl:value-of select="$ernests[@id = $beginid]/current_observation/time_to_food"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$ernests[@id = $beginid]/current_observation/time_to_water"/>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:variable>
					
					<xsl:variable name="cg-dist">
						<xsl:choose>
							<xsl:when test="$ernests[@id = $crave-group/@begin]/is_hungry = 'true'">
								<xsl:value-of select="$ernests[@id = $crave-group/@begin]/current_observation/time_to_food"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$ernests[@id = $crave-group/@begin]/current_observation/time_to_water"/>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:variable>
					
					<time>
						<xsl:variable name="percent-dist" select="100.0 * number($dist) div number($cg-dist)" />
						<xsl:choose>
							<xsl:when test="$percent-dist &lt;= 100 and number($dist) != 1000">
								<xsl:value-of select="$percent-dist" />
							</xsl:when>
							<xsl:otherwise><xsl:text>Far</xsl:text></xsl:otherwise>
						</xsl:choose>
					...
					<xsl:choose>
						<xsl:when test="$ernests[@id = $beginid]/current_observation/color">
							<color><xsl:value-of select="$ernests[@id = $beginid]/current_observation/color"/></color>
						</xsl:when>
						<xsl:otherwise>
							<color>008000</color>
						</xsl:otherwise>
				...
				<xsl:variable name="cond">
					<xsl:choose>
						...
						<xsl:when test="$pos != 1">
							...
							<xsl:value-of select="$equal-path" />
							<xsl:text> or count(</xsl:text>
							<xsl:value-of select="$equal-path" />
							<xsl:text>) != count($nodes[$posm1]/</xsl:text>
							<xsl:value-of select="$equal-path" />
							<xsl:text>)</xsl:text>
						</xsl:when>
		...
		<xsl:for-each select="$features_bounds">
			<xsl:if test="position() mod 2 = 1">
				...
				<group>
					...
					<!--<xsl:copy-of select="dyn:evaluate(concat('$nodes[', $date-path, ' &lt; $end-date and ', $date-path, ' &gt; $begin-date]'))"/>-->
				</group>
			</xsl:if>
		</xsl:for-each>
